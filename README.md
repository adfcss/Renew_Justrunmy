...
- 🖱️ **物理模拟**：Xvfb 虚拟桌面 + xdotool 鼠标物理点击模拟，强力穿透 Cloudflare Turnstile 人机验证。
- 🌐 **全协议代理**：内置 sing-box 核心，支持 vless/vmess/tuic/hy2/socks5/http 等协议。
...
2. **随机延迟**：每个账号独立延迟，范围为 **2~6 小时 (7200~21600 秒)**。
   - 示例：账号A在 08:00 签到，账号B在 11:30 签到，账号C在 14:15 签到。
   - 避免所有账号在同一时间段集中签到。
3. **乱序执行**：所有账号索引随机打乱执行，模拟真实用户行为。
4. **代理枢纽**：通过 `proxy_handler.py` 将 `PROXY_URL` 转为本地 `127.0.0.1:8080` 代理。
