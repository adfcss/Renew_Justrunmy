...
def handle_turnstile(sb):
    ...
    for i in range(1, 11):
        ...
        try:
            ...
            if c:
                bar = wi["oh"] - wi["ih"]
                _xdotool_click(c["cx"] + wi["sx"], c["cy"] + wi["sy"] + (bar if bar > 0 else 0))
        except: pass
        time.sleep(5)
    return False

def safe_js_fill(sb, selector, text):
    sb.execute_script("document.querySelector(arguments[0]).value = arguments[1];", selector, text)
    sb.execute_script("document.querySelector(arguments[0]).dispatchEvent(new Event('input', {bubbles:true}));", selector)
